...
public class SkillEffectWeaponAttackMultiplier : ISkillEffect
{
    int _multiplier;

    public SkillEffectWeaponAttackMultiplier(int multiplier)
    {
    ...
    public void ProcessEffect(CharacterObject c, List<ClickableTarget> hitCharacters, Vector4 point)
    {
        ...
        foreach (ClickableTarget clickable in hitCharacters)
        {
            if (clickable is CharacterObject)
            {
                ...
                int totalDamage = hitCharacter.MyCharacter.ApplyDamage(c.MyCharacter.Weapon.Damage * _multiplier);
    ...
    {
        return string.Format("Deal damage equal to your weapon's attack stat ({0}) x {1}: {2}", c.MyCharacter.Weapon.Damage, _multiplier, c.MyCharacter.Weapon.Damage * _multiplier);
    }
}
